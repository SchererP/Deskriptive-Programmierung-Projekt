...
generiereFeld s = let definiereFeld = \pos zahl feld -> setProtected (setField feld pos zahl) pos
                  in
                    definiereFeld (1,8) 1 (definiereFeld (2,1) 4 (definiereFeld (3,2) 2 (definiereFeld (4,5) 5 (definiereFeld (4,7) 4 (definiereFeld (4,9) 7 (definiereFeld (5,3) 8
                    (definiereFeld (5,7) 3 (definiereFeld (6,3) 1 (definiereFeld (6,5) 9 (definiereFeld (7,1) 3 (definiereFeld (7,4) 4 (definiereFeld (7,7) 2 (definiereFeld (8,2) 5
...
ueberpruefeFeld feld = let foldfunction =
                            \x y -> case x of
                                Nothing -> Nothing
                                Just xbool -> case y of
                                    Nothing -> Nothing
                                    Just ybool -> Just (ybool && xbool)
                           blockerstellung =
...
primes : List(Int)
primes = 1 :: (primesBelow 24)

disjunkteListe : List(Maybe Int) -> Maybe Bool
disjunkteListe x = let foldfunction =
                        \y x-> case y of
                            Just i-> case head(drop i primes) of --i-te Primzahl
                                Just item->item*x
                                Nothing -> 29*x --Sollte nie passieren, 29 ist die nächstgtößere Primzahl
                            Nothing -> 0
                   in case foldr foldfunction 0  x of
                        0 -> Nothing --Mindestens ein Nothing, ein nicht gefülltes Feld
                        223092870 -> Just True --Produkt der ersten neun Primzahlen, somit eindeutig
                        _ -> Just False --Mindestens eine Zahl doppelt

moeglicheZiffern : Sudokufeld -> Pos -> List Int
moeglicheZiffern feld pos = let
                                feldZuteilung = ((fst pos - 1) // 3)*3 + ((snd pos - 1) // 3 + 1)
                               in let filterbedingung y =
